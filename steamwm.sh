...
#
#
# $ DEBUGGER="$(pwd)/steamwm.sh" steam
#
# *or*
#
# $ ./steamwm.sh steam                     // Prints ld.so errors on 64-bit systems
#
# *or*
#
# $ ./steamwm.sh                           // Compile
...
mkdir -p "$out"
if [ "$cpp_file" -nt "$out/$soname" ] ; then
	echo "Compiling $soname..."
	g++ -shared -fPIC -m32 -x c++ "$cpp_file" -o "$out/$soname" \
